...
public class PuttingItAllTogetherTest extends EmbersAcceptanceTestBase {
    ...
    public void maintainAReportAgainstADatabase() throws Exception {
        givenARentalDatabase();
        whenAUserQueryIsCreatedThatWill("Find number of times each item has been rented");
        thenTheUserCanRunTheQuery();

        whenTheUserWantsTheQueryChangedSoThatItWill("Find number of times each item has been rented, sorted least rented first");
        thenTheUserCanRunTheQuery();

        then(theUserChecksTheQueriesPerformance(), andIsHappyItsFastEnough());
    }

    private void whenAUserQueryIsCreatedThatWill(String description) throws Exception {
    ...
    private void whenTheUserWantsTheQueryChangedSoThatItWill(String description) throws Exception {
        httpPost = new YatspecHttpPostCommandBuilder(super.interactions)
                .withUrl(embersServer.embersAdminPath())
                .withQueryName(QUERY_MOST_RENTED_ITEMS)
                .withQuerySql(
    ...
    private void givenARentalDatabase() {
    ...
    private ResultSetWrapper theUserChecksTheQueriesPerformance() {
    ...
    private void logSql(String logName, String sql) {
        try (Handle handle = embersServer.getEmbersDatabase().openDatabaseHandle()) {
            super.interactions.log(logName, new ResultSetWrapper(handle.createQuery(sql).list()));
        }
    }
}
