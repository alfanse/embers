...
@Notes("As a developer on a app with a relational database,\n" +
        ...
        "so that users can acces the information they need directly.")
public class PuttingItAllTogetherTest extends EmbersAcceptanceTestBase {
    ...
    public void maintainAReportAgainstADatabase() throws Exception {
        given(anARentalDatabase());
        when(aUserQueryIsCreated());
        thenTheUserRunsTheQuery();

        when(theUserWantsTheQueryOrderedAscending());
        thenTheUserRunsTheQuery();
    }

    private ActionUnderTest aUserQueryIsCreated() {
        httpPost = new YatspecHttpPostCommandBuilder(this)
                ...
                .withQuerySql(
                        ...
                        " group by i.name" +
                        " order by 2 desc ")
                .withQueryDescription("Find number of times each item has been rented")
                .build();
        httpPost.setLogPrefix("Create Query - ");
        return httpPost;
    }

    private void thenTheUserRunsTheQuery() throws Exception {
    ...
    private ActionUnderTest theUserWantsTheQueryOrderedAscending() {
        httpPost = new YatspecHttpPostCommandBuilder(this)
                .withUrl(embersServer.embersAdminPath())
                .withQueryName(QUERY_MOST_RENTED_ITEMS)
                .withQuerySql(
                        "select i.name, nvl(count(r.item_id),0) as \"Rented Count\" "+
                                ...
                                " group by i.name" +
                                " order by 2 asc ")
                .withQueryDescription("Find number of times each item has been rented sorted least rented first")
                .build();
    ...
    private GivensBuilder anARentalDatabase() {
        ...
        return givens -> {
            logSql("Items", "select * from items");
            logSql("Customers", "select * from customers");
            logSql("Rentals", "select * from rentals");
            return givens;
        };
    }
